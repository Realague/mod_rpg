...
public class BlockBarrelRhum extends Block{
public static final PropertyInteger LEVELS = PropertyInteger.create("levels", 0, 16);
	
	public BlockBarrelRhum(Material materialIn, String name) {
		super(Material.wood);
		this.setUnlocalizedName(name);
		this.setCreativeTab(CreativeTabs.tabFood);
		this.setDefaultState(this.blockState.getBaseState().withProperty(LEVELS, Integer.valueOf(0)));
	}
	                ...
	                else
	                {
	                    ...
	                    if (item == Items.glass_bottle)
	                    {
	                        if (i > 0)
	                        {
	                            if (!playerIn.capabilities.isCreativeMode)
	                            {
	                                itemstack1 = new ItemStack(ModItems.bottle_rhum,1,0);
