...
public class BlockOrge extends CropsBlock{
	   
	   public BlockOrge(String name) {
		super(name);
	}
	...
	@Override
	   protected Item getSeed()
	    {
	        return ModItems.seeds_orge;
	    }
	   
	   @Override
	    protected Item getCrop()
	    {
	        return ModItems.orge;
	    }
	}
/*public class BlockOrge extends BlockBush implements IGrowable {
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, BlockPos pos) {
		return new ItemStack(ModItems.seeds_orge);
	}
		...
		if(state.getValue(STAGE) == MAXSTAGE) {
			...
			for(int i = 0; i < seedAmount; i++) {
				if(rand.nextInt(15) < 7) {
					ret.add(new ItemStack(ModItems.seeds_orge));
				}
			}
		}
		else
