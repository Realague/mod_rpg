...
public class ItemHoeCustom extends SimpleItem{
	
	protected com.mod_rpg.Item.CustomToolMaterial theToolMaterial;
	private static final String __OBFID = "CL_00000039";
	 
	public ItemHoeCustom(CustomToolMaterial material, String name,int manche)
    {
		super (name);
        this.theToolMaterial = material;
        maxStackSize = 1;
        setMaxDamage(material.getMaxUses()+ manche);
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else
        {
            ...
            if (side != EnumFacing.DOWN && worldIn.isAirBlock(pos.up()))
            {
                ...
                {
                    switch (ItemHoeCustom.SwitchDirtType.TYPE_LOOKUP[((BlockDirt.DirtType)iblockstate.getValue(BlockDirt.VARIANT)).ordinal()])
                    {
                        case 1:
                            return this.useHoe(stack, playerIn, worldIn, pos, Blocks.farmland.getDefaultState());
                        case 2:
